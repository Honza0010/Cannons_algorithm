...
void testSequenceMultiplication(int matrixSize, double& sequentialTime)
{
  /*  ArrayMatrix<int> A_(matrixSize), B_(matrixSize), C_(matrixSize);
    A_.randFillMatrix();
    B_.randFillMatrix();*/
    ArrayMatrix<int> A_("ArrayMatrix.txt"), B_("ArrayMatrix.txt"), C_(matrixSize);
    TimerRT timer;
...
int main(int argc, char* argv[])
{
    ...
    TimerRT timer;
    int rows = 256*16;
    int cols = rows;     //Matrix size  /   Rozměry matice

    int rank, P;                //The process number and total number of processes  /   Cislo procesu a pocet procesu

    const int dimension = 2;    //Dimension of cartesian coordinates system of processes    /    Dimenze kartézského systému souřadnic procesů
    int dims[2];                //The number of processes along x and y axis    /    Velikost po osách x a y v systému procesů. dims[0] == dims[1] je nutnost.
    const int periodic[] = { 1,1 };     //osy x a y jsou zabaleny, tedy pokud např. proces 2 je úplně nejpravější proces po ose x a proces 1 nejlevější proces po ose x, pak pravý soused procesu 2 je proces 1 a levý soused procesu 1 je proces 2
                                        //Axis x and y are wrapped around.  The left neighbour of the process on the left border is the process on the right border etc...
    int blockSize;              //The size of the submatrix (rows / sqrt(P))    /      Velikost jednoho bloku (submatice) (rows / sqrt(P))
    int procDim;               //The size of cartesian coordinate system of the processes (sqrt(P))     /       sqrt(P), rozměry kartézského systému souřadnic pro procesy
    ...
    std::vector<int> commonData(4, 0); // {rows, cols, blockDim, blocksize} - master process (0) přepošle všechna tato data pomocí MPI_Bcast do všech ostatních procesů
                                        //Data sent to all processes via MPI_BCast
    ...
    MPI_Comm newCommunicator;     //Komunikátor pro nový MPI grid
                                  //Communicator for cartesian grid 

    int** A = nullptr, ** B = nullptr, ** C = nullptr;      //C = A*B, matice, které chceme počítat /   matrices that we want to calculate with
    int** subA = nullptr, ** subB = nullptr, ** subC = nullptr; //submatrices handled by individual processes
                                                                //Submatice procesů
    ...
    if (rank == 0)
    {
        ...
        try
        {
            //A = createMatrix<int>(rows, cols, 0);   //Allocation of matrices and random inicialization
            //B = createMatrix<int>(rows, cols, 0);
            //randFillMatrix(A, rows, cols);
            //randFillMatrix(B, rows, cols);
            //C = createMatrix<int>(rows, cols, 0);

            A = readMatrix<int>("matice.txt", rows, cols);    //Reading matrices from file
            B = readMatrix<int>("matice.txt", rows, cols);
            C = createMatrix<int>(rows, cols, 0);
    ...
    MPI_Scatterv(rank == 0 ? &(B[0][0]) : nullptr, sendcounts, displs, submat, &(subB[0][0]), blockSize * blockSize, MPI_INT, 0, MPI_COMM_WORLD);
     ...
     /*if (rank == 3 && blockSize <= 8)     //Náhodná kontrola, zda daný proces obdržel správnou submatici  /   random check whether the processes got the right submatrix
     {
         std::cout << "Process " << rank << " received localA" << std::endl;
         printMatrix(subA, blockSize, blockSize);
     }*/

     //Initial shifting along x by i left    /   Počáteční rozmístění submatic do procesů
    for (int i = 0; i < coords[0]; i++)
    {
        MPI_Sendrecv_replace(&(subA[0][0]), blockSize * blockSize, MPI_INT, left_rank, 1, right_rank, 1, newCommunicator, MPI_STATUS_IGNORE);
    }
    // Initial shifting along y by i up
    ...
    //Help matrices for sending of submatrices subA and subB if the function MPI_Sendrecv_replace was not available   
    // pomocné matice pro ukládání a posílání submatic subA a subB, kdyby funkce MPI_Sendrecv_replace nebyla dostupná
    ...
    for (int steps = 0; steps < procDim; steps++)
    {
        ...
        //This section shows how to handle submatrices shifting if the MPI_Sendrecv_replace was not available
        // Výměna submatic mezi procesy bez užití MPI_Sendrecv_replace
        //if (coords[1] == 0)     
        //{
        //    //sendVal = rank;
    ...
    //Deallocation of all matrices and arrays created during the algorithm
    deleteMatrix(subA);
    deleteMatrix(subB);
    deleteMatrix(subC);
    if (rank == 0)
