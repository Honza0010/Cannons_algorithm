...
int main(int argc, char* argv[])
{
    ...
    ArrayMatrix<int>* A = nullptr, * B = nullptr, * C = nullptr;
    ...
    if (rank == 0)
    {
        ...
        C = new ArrayMatrix<int>(rows);

        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < cols; j++)
            {
                A->setElement(i, j, i * rows + j);
                B->setElement(i, j, i * rows + j);
            }
        }
        C->setValue(0);
    ...
    ArrayMatrix<int> subA(blockSize), subB(blockSize), subC(blockSize);
    subC.setValue(0);
    ...
    for (int steps = 0; steps < procDim; steps++)
    {
        ...
        MPI_Sendrecv_replace(&(subA.getMatrix()[0]), blockSize * blockSize, MPI_INT, left_rank, 1, right_rank, 1, newCommunicator, MPI_STATUS_IGNORE);    //Shifting by 1 left along x axis  /   posunutí o jedna vlevo podél osy x
        MPI_Sendrecv_replace(&(subB.getMatrix()[0]), blockSize * blockSize, MPI_INT, up_rank, 1, down_rank, 1, newCommunicator, MPI_STATUS_IGNORE);       //Shifting by 1 up along y axis /   posunutí o jedna nahoru podél osy y
